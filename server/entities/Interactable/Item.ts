...
const modelForItemType: Record<ItemType, EntityModel[]> = {
	armor: [{ modelId: "armor", scale: 0.5, offset: [0, -1.25, 0] }],
	gamer_armor: [{ modelId: "gamer_armor", scale: 0.5, offset: [0, -1.25, 0] }],
	bow: [{ modelId: "bow", scale: 0.5, offset: [0, -1.4, 0] }],
	gamer_bow: [{ modelId: "gamer_bow", scale: 0.5, offset: [0, -1.65, 0] }],
	...
	magic_sauce: [
		{
			...
			scale: 0.5,
			offset: [0, -0.5, 0],
			rotation: [0, Math.cos(Math.PI / 8), 0, Math.sin(Math.PI / 8)],
		},
	],
	...
	wood: [{ modelId: "wood", scale: 0.5, offset: [0.5, 0, 0], rotation: [0, 0, Math.SQRT1_2, Math.SQRT1_2] }],
};
const colliderShapeForItemType: Record<ItemType, phys.Shape> = {
	armor: new phys.Box(new phys.Vec3(0.5, 0.4, 0.3)),
	gamer_armor: new phys.Box(new phys.Vec3(0.5, 0.4, 0.3)),
	bow: new phys.Box(new phys.Vec3(0.7, 2, 0.2)),
	gamer_bow: new phys.Box(new phys.Vec3(0.7, 2, 0.2)),
	sword: new phys.Box(new phys.Vec3(0.5, 1.5, 0.1)),
	gamer_sword: new phys.Box(new phys.Vec3(0.5, 1.5, 0.1)),
	axe: new phys.Box(new phys.Vec3(0.5, 1, 0.1)),
	iron: new phys.Box(new phys.Vec3(0.35, 0.6, 0.15)),
	knife: new phys.Box(new phys.Vec3(0.5, 1, 0.1)),
	magic_sauce: new phys.Cylinder(0.4, 0.3, 0.6, 4),
	mushroom: new phys.Sphere(0.4),
	pickaxe: new phys.Box(new phys.Vec3(0.5, 1, 0.1)),
	raw_iron: new phys.Sphere(0.5),
	shears: new phys.Box(new phys.Vec3(0.5, 0.6, 0.1)),
	string: new phys.Cylinder(0.4, 0.4, 1.2, 8),
	wood: new phys.Cylinder(0.3, 0.3, 1.4, 8),
};
export class Item extends InteractableEntity {
	...
	bind(player: PlayerEntity) {
		...
		// Entity should still exist so it can be taken from others' hands
